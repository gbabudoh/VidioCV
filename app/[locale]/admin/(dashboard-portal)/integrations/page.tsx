...
export default function AdminIntegrationsPage() {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Infrastructure</p>
          </div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">Global Integration Health</h1>
          <p className="text-slate-500 font-medium mt-1">Real-time monitoring of all third-party enterprise connections.</p>
        </div>
        <button 
          onClick={fetchGlobalIntegrations}
          className="flex items-center gap-2.5 px-6 py-3 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-800 transition-all active:scale-95 shadow-sm"
        >
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-[32px] border border-slate-100 shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <input 
            ...
            className="w-full pl-12 pr-6 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 text-sm font-medium focus:outline-none focus:ring-4 focus:ring-[#F7B980]/5 focus:border-[#F7B980]/50 transition-all placeholder:text-slate-300"
          />
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Employer Node</th>
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Protocol</th>
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Signal Status</th>
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Last Sync</th>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-8 py-20 text-center">
                      ...
                      <p className="text-slate-400 font-bold">No active integration signals found in this sector.</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    >
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-black text-slate-800 tracking-tight">{item.employer.name}</p>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{item.employer.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          <button className="p-2 bg-white hover:bg-slate-50 rounded-lg border border-slate-200 text-slate-400 hover:text-slate-800 transition-all cursor-pointer">
      ...
      {/* Audit Banner */}
      <div className="p-8 rounded-[40px] bg-slate-900 border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl">
        ...
        <button className="px-8 py-4 bg-white/10 hover:bg-white/20 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white transition-all cursor-pointer shadow-lg">
